...
const Payment: React.FC = () => {
  ...
  const fetchPaymentRequests = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3100/api/payment-requests/admin/requests', {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleViewRequest = async (requestId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3100/api/payment-requests/admin/requests/${requestId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleUpdatePayment = async () => {
    try {
      ...
      console.log('Full API URL:', `http://localhost:3100/api/payment-requests/admin/requests/${editingRequest._id}`);

      // Call update payment API
      const response = await fetch(`http://localhost:3100/api/payment-requests/admin/requests/${editingRequest._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const fetchStatistics = async () => {
    try {
      ...
      const url = `http://localhost:3100/api/payment-requests/admin/statistics${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const handleSubmitMarkReceived = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3100/api/payment-requests/admin/requests/${isMarkingPaymentReceived}/mark-paid`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
